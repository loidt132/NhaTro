...
export default function PaymentsReport(){
  ...
  const rows = filtered.map((i, idx)=>{
    const r = rooms.find(x=>x.id===i.roomId); const t = tenants.find(x=>x.id===i.tenantId); const paid = paidAmount(i, payments);
    return [idx+1, r?.name||'', t?.name||'', i.month, (i.total||0).toLocaleString(), paid.toLocaleString()];
  });
  const sumTotal = filtered.reduce((a,i)=> a + (+i.total||0), 0);
  const sumPaid = filtered.reduce((a,i)=> a + paidAmount(i, payments), 0);
  const onExport = ()=> exportReportPdf({ title: 'BÁO CÁO TIỀN ĐÃ ĐÓNG', subtitle: mode==='month'?`Tháng: ${sel.month}`:`Năm: ${sel.year}`, columns: ['#','Phòng','Khách','Kỳ','Tổng hóa đơn','Đã đóng'], rows, summary: [`Tổng hóa đơn: ${sumTotal.toLocaleString()} đ`, `Đã đóng: ${sumPaid.toLocaleString()} đ`], fileName: `payments-${mode==='month'?sel.month:sel.year}.pdf` });
  return (
