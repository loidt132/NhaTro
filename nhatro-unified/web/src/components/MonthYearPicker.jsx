...
export default function MonthYearPicker({ value, mode = 'month', onChange }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      <select className="h-[29px] border rounded px-2 text-xs" value={value?.year || String(now.getFullYear())} onChange={e=>handleYear(e.target.value)}>
        {years.map(y => <option key={y} value={y}>{y}</option>)}
      </select>
      {mode === 'month' && (
        <select className="h-[29px] border rounded px-2 text-xs" value={value?.month?.split('-')[1] || String(now.getMonth()+1).padStart(2,'0')} onChange={e=>handleMonth(e.target.value)}>
