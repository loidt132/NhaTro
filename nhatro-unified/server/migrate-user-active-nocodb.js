...
if (!baseUrl || !token || !tableId) throw new Error('Thiếu cấu hình NocoDB users.');
...
(async () => {
  const columns = await request(`${baseUrl}/api/v2/meta/tables/${encodeURIComponent(tableId)}/columns?limit=1000`);
  const exists = (columns.list || []).some((column) => ['isActive', 'is_active', 'active'].includes(String(column.title || '')));
